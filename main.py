...
def process_chat_message(message):
    ...
    try:
        ...
        if bot:
            ...
            if execution_time > 25:  # Warn if approaching limit
                print(f"⚠️ Warning: Function execution time: {execution_time:.2f}s (approaching limit)")
...
<body class="bg-zinc-950 text-zinc-100 min-h-screen">
    ...
    <script>
        ...
        fetch('/health', {
            ...
            env.textContent = 'Status: Google Cloud Function Active';
        });
...
def chat(request):
    """Google Cloud Function HTTP handler for chat requests"""
    ...
    try:
        ...
        if request.method == 'POST':
            # Check payload size (Google Cloud limit: 10MB)
            body_size = len(request.get_data())
            if body_size > 10 * 1024 * 1024:  # 10MB in bytes
                return (json.dumps({"error": "Request payload too large (max 10MB)"}), 413, headers)
            
            data = request.get_json()
            message = data.get('message', '').strip() if data else ''
            
            if not message:
                return (json.dumps({"error": "Empty message"}), 400, headers)
            ...
            if len(response_json) > 10 * 1024 * 1024:  # 10MB in bytes
        ...
        # Handle GET requests
            ...
            if path.endswith('/health'):
                return (json.dumps({
                    ...
                    "status": "Google Cloud Function Active"
                }), 200, headers)
            elif path.endswith('/test'):
                return test_vertex_ai()
            else:
                # Return the HTML page for root requests
                return (get_chat_html(), 200, {'Content-Type': 'text/html'})
...
def test_vertex_ai():
    ...
    try:
        ...
        try:
            ...
            return (json.dumps({"error": f"vertexai import failed: {e}"}), 500, headers)
        
        # Test initialization
        bot = initialize_vertex_ai()
        if bot:
            return (json.dumps({"status": "All tests passed", "vertex_ai": True}), 200, headers)
        else:
            return (json.dumps({"error": "Failed to initialize Vertex AI", "vertex_ai": False}), 500, headers)
            
    except Exception as e:
        return (json.dumps({"error": f"Test failed: {e}"}), 500, headers)
